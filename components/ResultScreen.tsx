...
const ResultScreen: React.FC<ResultScreenProps> = ({ record, onDone, onHome, onChat }) => {
  const { t } = useTranslation();
    
  const handleDownloadPdf = () => {
    generatePdfReport(record, t);
  };
  ...
  return (
    <div className="min-h-screen bg-base-200">
      <header className="bg-base-100 shadow-md p-4 flex justify-between items-center sticky top-0 z-10">
        <button onClick={onHome} className="p-2 rounded-full hover:bg-base-300">
        ...
        <div className="max-w-4xl mx-auto space-y-6 pb-40">
          <div className={`p-6 rounded-2xl shadow-lg ${summaryStyle.bg} ${summaryStyle.text}`}>
              ...
              <p className="mt-2 text-sm">
                  {t('result.reportSummary')} {record.date.toLocaleDateString()}.
              </p>
          </div>
          ...
          {onChat && (
            <button 
              onClick={onChat}
              className="w-full p-4 bg-gradient-to-r from-blue-600 to-blue-500 rounded-2xl shadow-lg flex items-center justify-between text-white hover:shadow-xl transition-all"
            >
